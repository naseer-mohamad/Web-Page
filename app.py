...
def api_generate_ai():
    """Generate quiz/questions using Google Generative Language API via REST.
    ...
    body = {
        'prompt': {
            'messages': [
                {'author': 'user', 'content': {'text': instruction}}
            ]
        },
        'temperature': 0.2,
    ...
    # Use the installed google.generativeai client library
    try:
        ...
        for m in available:
            ...
            if 'generateContent' in methods:
                ...
                break
        if not chosen_model:
            return jsonify({'success': False, 'message': 'No generative model available for this API key'}), 502

        model = genai.GenerativeModel(chosen_model)
        # Ask the model to produce the structured JSON
        resp = model.generate_content(instruction)
        generated_text = getattr(resp, 'text', '') or ''
    ...
    try:
        ...
        return jsonify({'success': True, 'raw': generated_text})
